...
async def _batch(event):
    ...
    async with gagan.conversation(event.chat_id) as conv: 
        if not s:
            ...
            cancel_flag[event.sender_id] = False
            ...
            ids.clear()
            batch.clear()
            del cancel_flag[event.sender_id]

@gagan.on(events.callbackquery.CallbackQuery(data="cancel"))
async def cancel(event):
    user_id = event.sender_id
    cancel_flag[user_id] = True
    await event.respond("Batch process has been cancelled.")

async def run_batch(userbot, client, sender, countdown, link):
    for i in range(len(ids)):
        if cancel_flag.get(sender):
            await client.send_message(sender, "Batch process has been cancelled.")
            return
        timer = 6
        if i < 250:
            timer = 2
...
TEXT = "👋 Hi, This is `Restricted Content Saver bot` \n\n\nMade with ❤️\nMade by __**MAHI Botz❤️‍🔥🫰**__."

@gagan.on(events.NewMessage(pattern=f"^{C}"))
async def start_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.inline("✖️ Cancel ✖️", data="cancel"),
