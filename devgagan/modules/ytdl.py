...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"@Mr_MAHIji{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by MAHI BOTz")
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        ...
        title = info_dict.get('title', 'Powered by MAHI BOTz')
